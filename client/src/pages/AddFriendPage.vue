...
const handleSearch = async (e) => {
  if (e.key === 'Enter') {
    ...
    try {
      ...
      const res = await fetchData(`${userId}/search?query=${searchValue}`, 'GET', '')
      const data = await res?.json()

      if (data.message) {
        searchedUser.value.connectionFirstName = data.message.firstName
        searchedUser.value.connectionLastName = data.message.lastName
        searchedUser.value.connectionEmail = data.message.email
        searchedUser.value.connectionUserId = data.message.userId
        searchedUser.value.connectionConnected = data.message.connected
      } else {
        searchedUser.value = {
          connectionFirstName: '',
          connectionLastName: '',
...
const handleUpdateConnection = ({ userId, connected }) => {
  if (searchedUser.value.connectionUserId === userId) {
    searchedUser.value.connectionConnected = connected
  }
}
</script>
  ...
  <div class="layout">
    <div class="fixed">
      <div class="header-container">
        ...
        <p class="header">Build your circle</p>
      </div>
